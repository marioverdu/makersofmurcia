...
export const ChatTuentiButtonV2: React.FC<ChatTuentiButtonV2Props> = ({
  ...
  return (
    <>
      {/* Chat button - Réplica exacta del HTML real */}
      {showChatButton && (
        <button
          onClick={toggleChat}
          className={`fixed ${buttonPositionClasses} z-[1000] w-9 h-9 bg-transparent rounded-full flex items-center justify-center shadow-lg transition-all duration-200 ${isBlinking ? "msn-blinking" : ""} ${customClassName}`}
          style={{
            ...
            left: position.includes("left") ? `${padding.left}px` : "auto",
            top: position.includes("top") ? `${padding.top}px` : "auto",
          }}
          aria-label={ariaLabel || (isOpen ? "Close chat" : "Open chat")}
        >
          <img 
            alt={isOpen ? "Cerrar chat Tuenti" : "Abrir chat Tuenti"}
            className="w-9 h-9 object-contain" 
            draggable="false" 
            src={isOpen 
              ? "https://assets.marioverdu.com/button/chat/chat-close.svg"
              : "https://assets.marioverdu.com/button/chat/chat-tuenti-button.svg"
            }
          />
        </button>
      )}
...
export default function ChatTuentiButtonMaster(props: any) {
  return <ChatTuentiButtonV2 {...props} />
}
